...
if (!$errors) {	
	...
	$message .= "--\n";
	if (file_exists($file)) {
		$message .= $_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']) . '/' . $file;
	} else {
	...
	// Confirmation	is the same in this demo as content above now
	$user = $_POST['email'];
	...
	****/
	...
	// Send the same content in this demo to the user too
